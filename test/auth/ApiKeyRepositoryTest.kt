...
class ApiKeyRepositoryTest: DBTest() {
  ...
  @Test fun `create and find by key`() {
    val key = ApiKey(user.id, "st-abc123")
    repo.create(key)
    expect(key.id).notToEqual(Id(0))
    expect(repo.byKey("st-abc123")?.id).toEqual(key.id)
  }

  @Test fun `list for user`() {
    val key1 = ApiKey(user.id, "st-key1")
    val key2 = ApiKey(user.id, "st-key2")
    repo.create(key1)
    repo.create(key2)
  ...
  @Test fun `delete for user`() {
    val key = ApiKey(user.id, "st-to-delete")
    repo.create(key)
    repo.deleteForUser(key.id, user.id)
    expect(repo.byKey("st-to-delete")).toEqual(null)
  }

  @Test fun `update last used`() {
    val key = ApiKey(user.id, "st-used")
    repo.create(key)
    expect(key.lastUsedAt).toEqual(null)
    repo.updateLastUsed(key.id)
    val updated = repo.byKey("st-used")
